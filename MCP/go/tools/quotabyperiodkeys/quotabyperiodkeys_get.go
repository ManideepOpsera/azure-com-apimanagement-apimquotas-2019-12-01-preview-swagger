...
func Quotabyperiodkeys_getHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateQuotabyperiodkeys_getTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_subscriptions_subscriptionId_resourceGroups_resourceGroupName_providers_Microsoft_ApiManagement_service_serviceName_quotas_quotaCounterKey_periods_quotaPeriodKey",
		mcp.WithDescription("Gets the value of the quota counter associated with the counter-key in the policy for the specific period in service instance."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Quotabyperiodkeys_getHandler(cfg),
	}
}
